...
# (1) Obtains phase II and III hapmap genotype data for CEU and YRI populations.
# and saves as CEU_Gen_Dat.Rda
# (2) Excludes some snps based on potential errors as specified by SNPs_to_Exclude.xlsx
# (3) Converted genotypes from ATGC format to 0, 1, or 2 representing the minor 
# allele frequencies as defined by the scraped ncbi data. Then columns were stored 
# as integer/numeric type, and the data was saved as CEU_Gen_Dat_Clean2.Rda
# (4) Combined phenotype data for Cyto, Repair, IC, Mut, and Mutation outcomes
# (5) log transformed the average IC measures (20, 50 and 80) and coded gender as a factor
# (6) Mut10v0, Mut20v0, and Mut20v10 were divided by 2, then 
# added to 0.01 and then log base 10 transformed.
# (7) Regression was then fit: phenotype ~ gender + snp for each snp, and saved 
# partitioned results in CEU_Mass_Uni_Results_p.rds for pth partition
...
#path <- "C:/Users/Owner/Google Drive/Genetics_Research/"
...
# Adding in cell turnover data
turnovers <- read_excel("New/cell turnover for biostat 081215.xlsx") %>% 
  filter(Race == population) %>% select(Name, `Turnover rate`) %>% 
  rename(Cell_line = Name, turnover = `Turnover rate`)

PhenData_Combined <- full_join(PhenData_Combined, turnovers, by = "Cell_line") %>% select(-Viability)
...
for(p in 1:20) {
  ...
  results <- foreach(j = 4:(ncol(PhenData_Combined) - 1)) %dopar% {
    library(data.table)
    library(tidyverse)
    # Creating function for fitting MLR model
    MLR <- function(gendCol, turnCol, snpCol, phenCol) {
      temp <- data.frame(g = gendCol, s = snpCol)
      if(temp[complete.cases(temp), c("g")] %>% unique() %>% length() > 1) {
        fit <- lm(phenCol ~ gendCol + turnCol + snpCol)
    ...
    tempResult <- lapply(FUN = MLR, X = as.data.frame(Gen_Dat)[, -c(1)], 
                         ...
                         turnCol = PhenData_Combined$turnover,
  ...
  saveRDS(results, paste0(getwd(), "/Analysis_Results/Turnover/", population, "_Mass_Uni_Results_", p, ".rds"))
}
